...
`/ai/cashflow/calculate`, `/ai/onboarding/{user_id}/status`, `/ai/bank-offers`, `/health`, `/ai/queue/status`
...
```json
{
  ...
  "context": {
    "history": [
      {"role": "user", "text": "Привет, расскажи о своих возможностях"},
      {"role": "assistant", "text": "Я финансовый помощник, помогу разобраться с бюджетом..."}
    ],
    "user_profile": {
      "age": 28,
...
История хранится на стороне Backend и передаётся в каждом запросе. AI-сервис не сохраняет её сам. LLM-аналитик видит историю и учитывает контекст предыдущих сообщений.
...
**Ошибки (HTTP 422):**
```json
{"detail": "Профиль не найден — сначала пройдите онбординг"}
{"detail": "Сначала пройдите онбординг"}
```
...
Подбор банковских предложений по кредиту. Без LLM и внешнего поиска — детерминированный скоринг по базе из 48 предложений 28 российских банков. Не использует очередь, отвечает мгновенно.
...
| `loan_amount` | float | да | Сумма кредита, руб — используется для расчёта `monthly_payment` |
| `loan_rate` | float | да | Желаемая ставка, % годовых — используется для скоринга |
| `loan_months` | int | да | Желаемый срок, месяцев — используется для скоринга |
  ...
  "offers": [
    {
      ...
      "rate": 13.9,
      "loan_months": 24,
      "monthly_payment": 23982.83,
      "score": 97,
      "logo_url": "https://img.logo.dev/tbank.ru?token=free",
      "offer_url": "https://tbank.ru/credits/cash/"
    },
    {
      "bank_name": "Сбербанк",
      "domain": "sber.ru",
...
Всегда возвращает ровно **6 офферов** — топ-6 по скору из базы 48 предложений.

**Поля ответа:**

| Поле | Тип | Описание |
|------|-----|----------|
| `offers` | array[6] | Топ-6 предложений, отсортированных по `score` убыванию |
| `search_query` | string | Человекочитаемое описание запроса пользователя |

**Поля одного предложения (`offers[i]`):**

| Поле | Тип | Описание |
|------|-----|----------|
| `bank_name` | string | Название банка на русском |
| `domain` | string | Корневой домен банка |
| `rate` | float | Ставка по предложению, % годовых |
| `loan_months` | int | Срок предложения, месяцев |
| `monthly_payment` | float | Аннуитетный платёж для `loan_amount` по данной ставке и сроку, руб |
| `score` | int | Релевантность 0–100 — насколько предложение близко к запросу |
| `logo_url` | string | Логотип через `img.logo.dev` |
| `offer_url` | string | Ссылка на страницу кредита |

**Формула скора:**
```
score = max(0, 1 − |offer.rate − loan_rate| / loan_rate) × 40
      + max(0, 1 − |offer.loan_months − loan_months| / loan_months) × 60
```
Ставка влияет на 40%, срок — на 60%. Предложение с точным совпадением по обоим параметрам получает score = 100.
...
Передавай `context.history` с историей диалога в каждом запросе — AI учтёт предыдущие сообщения.
...
```
POST /ai/bank-offers {"user_id": "...", "loan_amount": 500000, "loan_rate": 15.0, "loan_months": 24}
  → всегда 6 офферов, отсортированных по близости к запросу
  → logo_url — готовый URL логотипа для <img>
  → offer_url — кнопка "Перейти к офферу"
  → score — для выделения лучшего предложения (топ = ближайший по ставке и сроку)
```
Эндпоинт работает без LLM и без внешних запросов — мгновенный ответ.
...
|----------|-----|----------|---------|
| `POST /ai/cashflow/calculate` | 422 | `"Профиль не найден — сначала пройдите онбординг"` | Онбординг не пройден |
| `POST /ai/cashflow/calculate` | 422 | `"Сначала пройдите онбординг"` | Есть профиль, но нет `monthly_expenses_estimate` |

> **Важно для Backend:** HTTP 4xx от AI-сервиса — это бизнес-ошибки (не пройден онбординг, нет данных). Их нужно передавать пользователю через поле `detail`, а **не** оборачивать в `"AI service unreachable"`. `"Unreachable"` уместен только при сетевой ошибке или HTTP 5xx.
...
| `POST /ai/bank-offers` | < 50 мс (без LLM, без поиска) | 5 с |
